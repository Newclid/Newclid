...
namespace Yuclid {
  ...
  class NonPerpendicular : public Statement {
  public:
    NonPerpendicular(const SlopeAngle &l, const SlopeAngle &r) : m_left(l), m_right(r) { }
    ...
    auto operator<=>(const NonPerpendicular &other) const = default;

    std::ostream &print(std::ostream &out) const override;

    [[nodiscard]] std::unique_ptr<Statement> clone() const override {
      return std::make_unique<NonPerpendicular>(*this);
    }

    ~NonPerpendicular() override = default;
